...
function createGameStore(rootDir) {
  ...
  function writeGame(id, game) {
    ensureDir();
    const { title, theme, shuffleQuestions, shuffleAnswers, questions } = game;
    fs.writeFileSync(
      gameFile(id),
      JSON.stringify({ title, theme: theme || '', shuffleQuestions, shuffleAnswers, questions }, null, 2),
      'utf8'
    );
  }
  ...
  function createGame(id, title) {
    ...
    writeGame(id, { title: title || id, theme: '', shuffleQuestions: false, shuffleAnswers: false, questions: [] });
  }
  function updateGameMeta(id, meta) {
    const g = getGame(id);
    if (meta.title != null) g.title = meta.title;
    if (meta.theme != null) g.theme = String(meta.theme);
